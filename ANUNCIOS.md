...
3. Pega el **snippet de verificación** en el `<head>` de `index.html` y `dashboard.html` (hay un hueco comentado).
4. **Espera la revisión** de Google (días a ~2-3 semanas; pide contenido real y algo de tráfico).
5. Aprobado → crea **bloques de anuncios** → copia tu `ca-pub-XXXX` y el `data-ad-slot`.
6. Descomenta el `<script ... adsbygoogle ...>` del `<head>` y mete el `<ins class="adsbygoogle" ...>` en cada `.adslot__box` (donde ahora sale el anuncio demo).
